...
def _find_project_in_attic(name: str) -> Path | None:
    """在 attic/ 中查找项目"""
    from ..config import find_workspace_root
...
def archive(
    ...
    reason: str | None = typer.Option(None, "--reason", "-r", help="归档原因"),
...
def unarchive(
    ...
    target_dir: str | None = typer.Option(None, "--target", "-t", help="目标目录 (apps 或 libs)，默认为自动检测"),
