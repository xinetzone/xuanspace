...
def _find_large_files(workspace_root: Path, threshold_mb: int = LFS_THRESHOLD_MB) -> list[tuple[Path, int]]:
    ...
    for item in workspace_root.rglob("*"):
        ...
        if lfs_patterns and not item.is_symlink():
            is_lfs_pattern = any(
                item.match(pattern) for pattern in lfs_patterns
            )
...
def lfs_check(
    threshold: int = typer.Option(
        LFS_THRESHOLD_MB, "--threshold", "-t", help="大文件阈值 (MB)",
    ),
    json_output: bool = typer.Option(
        False, "--json", "-j", help="JSON 格式输出",
    ),
) -> None:
    ...
    if json_output:
        ...
        result = {
            ...
            "large_files": [
                {"path": str(item[0].relative_to(root)), "size_mb": item[1]}
    ...
    console.print(table)
    console.print()
    console.print("[dim]提示: 运行 [bold]git lfs track <pattern>[/bold] 添加跟踪规则，然后 [bold]git add .gitattributes[/bold] 提交[/dim]")
