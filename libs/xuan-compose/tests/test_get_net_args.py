...
"""逐用例移植自上游 tests/unit/test_get_net_args.py（31 例，断言零削弱）。

适配点：
- ``podman_compose.get_net_args`` → ``xuan_compose.translate.networks.get_net_args``。
- 不再 import ``PodmanCompose``；``get_networked_compose`` 注解改为 ``Any``，
  helper 跨文件复用改为 ``from tests.test_container_to_args import create_compose_mock``
  （tests 目录扁平化：上游 tests.unit.* → tests.*）。
"""
...
from tests.test_container_to_args import create_compose_mock
from xuan_compose.translate.networks import get_net_args
...
def get_networked_compose(num_networks: int = 1) -> Any:
