...
"""逐用例移植自上游 tests/unit/test_depends_on.py（断言零削弱）。

适配点：扁平 ``podman_compose`` 导入改为分层路径
（``ServiceDependency`` → ``xuan_compose.model``；
``flat_deps``/``check_dep_conditions`` → ``xuan_compose.dependencies``；
``subprocess.CalledProcessError`` → ``xuan_compose.runner.CalledProcessError``）；
mock.patch("podman_compose.Podman") 在本文件中未使用，无需对应替换。
"""
...
from xuan_compose.dependencies import check_dep_conditions, flat_deps
...
class TestCheckDepConditions(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_retry_on_error(self) -> None:
        ...
        compose.podman.output.side_effect = [
            ...
            CalledProcessError(1, "podman wait", b"", b"error"),
