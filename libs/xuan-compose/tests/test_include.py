...
"""逐用例移植自上游 tests/unit/test_include.py（8 用例，断言零削弱）。

T3 延期登记：``_parse_compose_file`` 属引擎层，随 T6 迁入。

适配点：
- 扁平 ``podman_compose`` 导入改为 ``xuan_compose.engine.ComposeEngine``。
- 上游把 yaml 直接写到 pytest 启动目录、tearDown 按名删除；移植版改为每个
  用例在独立临时目录中 chdir 执行并快照/还原进程环境变量（引擎解析会读写
  ``os.environ`` 的 COMPOSE_*/PODMAN_* 键），断言数据与期望逐行不变。
"""
...
class TestIncludeDict(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self._tmp = tempfile.mkdtemp(prefix="xuan-compose-include-")
        os.chdir(self._tmp)
        self.test_files: list[str] = []
        self.podman_compose = ComposeEngine()
