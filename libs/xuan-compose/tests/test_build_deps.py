...
"""逐用例移植自上游 tests/unit/test_build_deps.py（4 用例，断言零削弱）。

T5 延期登记：被测符号 ``ComposeEngine._resolve_context_dependencies``
属引擎层，随 T6 迁入。唯一适配是扁平 ``podman_compose`` 导入改为
