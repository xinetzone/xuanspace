...
- 许可证：**GPL-2.0-only**（继承上游，不得重新许可）；完整许可证文本见 [LICENSE](LICENSE)；
  全部 95 个 `.py` 文件携带 `SPDX-License-Identifier: GPL-2.0-only` 头
...
flowchart TD
    ...
    CMD["commands/<br/>24 个命令 handler + COMMAND_HANDLERS 注册表"]
    ENG["engine.py<br/>ComposeEngine：compose 解析与状态装配"]
    EXE["执行层 runner.py · model.py · dependencies.py · pull.py · logs.py"]
    ...
    BASE["基础层 compat.py · errors.py · types.py · logging_utils.py"]
    ...
    RUNNER["runner.py：全包唯一子进程边界（import subprocess）"]
    EXE -.-> RUNNER
```

依赖严格单向（`runner ← 规范/翻译层 ← engine ← commands ← cli`），无反向边、无环；
