...
# 测试基础功能
handle = demo.tls_command_handle()
...
| 模式 | 用途 | 硬件要求 | 配置选项 |
|------|------|---------|---------|
| **Stub 模式** | 开发、单元测试、CI | 无（纯 CPU 模拟） | `-DDEMO_FFI_USE_STUB=ON`（默认） |
| **Real 模式** | 生产环境、真实硬件执行 | 需要真实硬件 | `-DDEMO_FFI_USE_STUB=OFF` |
...
# Real 模式
cmake -B build -G Ninja -DDEMO_FFI_USE_STUB=OFF
```
...
[PASS] Prefix match: demo
[PASS] All functions accessible: demo.tls_command_handle, demo.buffer_alloc, ...
```
...
编辑 `src/demo/ffi_registry.cc`，在 `TVM_FFI_STATIC_INIT_BLOCK()` 中添加：

```cpp
refl::GlobalDef()
    .def("demo.my_new_func", my_new_func_impl);
```
...
编辑 `python/demo_ffi/demo/__init__.py`，添加函数类型注解：

```python
def my_new_func(param1: int, param2: str) -> int:
    """我的新函数文档字符串。"""
    return _LIB._api_call("demo.my_new_func", param1, param2)
```
...
def test_my_new_func():
    result = demo.my_new_func(42, "hello")
