...
BUILD_MODULE_DIR="$BUILD_DIR/src/demo"
...
build_cpp() {
    ...
    if [ ! -d "$BUILD_DIR" ]; then
        ...
        (cd "$PROJECT_ROOT" && cmake -B build -G Ninja -DDEMO_FFI_USE_STUB=ON)
...
install_package() {
    step "Checking if demo_ffi is already installed..."
    if python_module_exists demo_ffi; then
        pass "demo_ffi already installed in editable mode"
    else
        step "Installing demo_ffi (editable, no-build-isolation)..."
        (cd "$PROJECT_ROOT" && pip install --no-build-isolation -e .)
        pass "demo_ffi installed"
    fi
}
...
verify_import() {
    step "Verifying demo_ffi import..."
    setup_library_path
    python -c "
import os, sys
...
show_help() {
    echo "demo_ffi Development Setup Script"
...
echo ""
echo -e "${CYAN}========================================${NC}"
echo -e "${CYAN}  demo_ffi Development Setup${NC}"
echo -e "${CYAN}========================================${NC}"
