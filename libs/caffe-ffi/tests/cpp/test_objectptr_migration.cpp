...
TEST(ObjectPtrMigration, RegistryHoldsOwnershipAfterSourceOutOfScope) {
...

  {
    ...
    obj->cpu_data()[0] = 99.0f;
    obj->cpu_data()[1] = 88.0f;
...
TEST(ObjectPtrMigration, VectorOfObjectPtrsBulkOperations) {
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    b->cpu_data()[0] = static_cast<float>(i * 10);
    b->cpu_data()[1] = static_cast<float>(i * 10 + 1);
    b->cpu_data()[2] = static_cast<float>(i * 10 + 2);
