...
> **类比理解**：想象一份会议纪要需要发给 100 人参阅。
> - **旧方案（memcpy）**：打印 100 份，人手一份 → 打印机累坏了，桌子堆满了
> - **COW 方案**：群里发一份链接，大家在线查看 → 零打印，零占用
> - 如果有人需要**修改**自己的那份（写操作），系统才会给他单独复制一份 —— 这就是"写时复制"名字的由来

在深度学习推理中，Split 层的作用就是"把数据分发给多个后续层"。在纯前向推理（Forward）场景下，后续层只**读取**数据而不修改它，所以 COW 能做到真正的**零拷贝**：

```
旧方案 (Phase 1 memcpy):                    COW 方案 (Phase 2 零拷贝):
                                          
  [data: 256KB]                             [data: 256KB] ← 同一份数据
      ↓ Split Forward                          ↓      ↓      ↓ (仅引用计数+1)
  ┌───┴───┬───────┐                        ┌───┴───┬───────┐
  ↓       ↓       ↓                        ↓       ↓       ↓
[copy] [copy] [copy]  ← 每个分支独占拷贝   [ref]  [ref]  [ref]  ← 共享同一份
256KB  256KB  256KB   共 768KB 内存         ref≈0  ref≈0  ref≈0   共 256KB 内存
~20μs ×3 ≈ 60μs    拷贝耗时               ~1μs ×3 ≈ 3μs     仅指针操作
```

**COW 的技术本质**：

- Blob 内部数据由 `ObjectPtr`（引用计数智能指针）持有，类似 C++ `std::shared_ptr`
- Split 的 `Forward` 调用 `ShareData()`：仅做指针赋值 + 原子递增引用计数（~1μs）
- 当某分支后续需要**修改**数据时（如 in-place ReLU），`mutable_data_tensor()` 检测到引用计数 > 1，触发**一次**真实 memcpy，然后该分支持有私有副本
- **纯推理场景下，所有分支均为只读消费者，永远不会触发写时复制 → 真正的零拷贝**

**一个容易误解的点**：COW 不是"从不复制"，而是"延迟到真正需要写的时候才复制"。在训练场景（需要反向传播写梯度）中，COW 会按需复制；但在推理场景中，Split 永远只读，因此零拷贝 100% 生效。
...
2. **推理阶段趋势**：
   ...
   - COW 零拷贝使得 Split 推理开销从 **O(数据量)**（搬数据）降为 **O(fan-out 数)**（数人头），单 Split 约 1μs/分支
   - Split 不再是推理瓶颈，即使在深度嵌套 Inception 网络（4 层 split × N=2）中，Split 总开销也 < 50μs
   - 分支数 N 翻倍时，memcpy 方案耗时翻倍，COW 方案仅多 1μs（多一个引用计数）

3. **内存趋势**（用"100 份文件"类比）：
   - Phase 1（memcpy）：N 个分支各存一份私有副本 → 内存 = N × 数据大小。N=8 时存 8 份，浪费 87.5%
   - Phase 2（COW）：N 个分支共享同一份数据 → 内存 = 1 × 数据大小。100 个分支也只存 1 份
   - 深度嵌套场景（4 splits × N=2）：COW 节省约 4×count×4B，相当于原本需要 8 份数据现在只存 1 份
   - **直觉**：分多少路都不增加内存，唯一的额外开销是每个分支一个指针（8 字节）
